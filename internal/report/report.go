...
func DefaultSinkFiles(outDir string) SinkFiles {
	return SinkFiles{
		...
		DNS:       filepath.Join(outDir, "dns", "dns.passive"),
		ActiveDNS: filepath.Join(outDir, "dns", "dns.active"),
	}
}

// Generate reads the artifact manifest and renders an HTML report in cfg.OutDir.
func Generate(ctx context.Context, cfg *config.Config, _ SinkFiles) error {
	if cfg == nil {
