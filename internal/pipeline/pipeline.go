...
type Sink struct {
	...
	artifactIndex         map[artifacts.Key]*artifactRecord
	artifactOrder         []artifacts.Key
...
func NewSink(outdir string, active bool, target string, lineBuffer int) (*Sink, error) {
	...
	s := &Sink{
		...
		artifactIndex:         make(map[artifacts.Key]*artifactRecord),
...
func (s *Sink) recordArtifact(tool string, artifact artifacts.Artifact) {
...
func (s *Sink) flushArtifacts() {
	...
	records := make([]artifacts.Artifact, 0, len(s.artifactOrder))
...
func handleMeta(s *Sink, line string, isActive bool, tool string) bool {
	...
	if strings.HasPrefix(trimmed, "meta:") {
		...
		s.recordArtifact(tool, artifacts.Artifact{
			Type:   "meta",
			Value:  normalized,
			Active: isActive,
			Up:     true,
...
func handleJS(s *Sink, line string, isActive bool, tool string) bool {
	...
	base := artifacts.ExtractRouteBase(js)
...
func handleHTML(s *Sink, line string, isActive bool, tool string) bool {
	...
	base := html
	if isActive {
		base = artifacts.ExtractRouteBase(html)
	}
